...
| U-7 | ✅ | ~~**Mode conduite**~~ | ✅ — panel `#drive-panel` fixé en bas (max 60 vh) ; bouton ⬆ affiché après calcul d'itinéraire si GPS actif ; `enterDriveMode()` / `exitDriveMode()` / `refreshDrivePanel()` ; 10 prochaines stations en liste verticale (plus proche en bas), layout horizontal par card : distance | opérateur + nom de station + puissance + dispo ; bordure bleue sur la prochaine, or si réseau préféré (F-9b) |
