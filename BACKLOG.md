...
| F-9b | 🔍 | **Réseau préféré en route** — l'utilisateur configure 1-2 réseaux pour lesquels il a un abonnement actif (persisté en localStorage) ; en mode conduite (U-7), les stations de ces réseaux sont visuellement distinguées parmi les prochaines options. Pas un filtre exclusif : les autres stations restent visibles. But : permettre un arbitrage praticité/prix rapide pendant la conduite. | Sous-feature de U-7, à concevoir conjointement ; les alliances tarifaires (ex : cocher "Ionity" inclut automatiquement Electra/Fastned/Atlante) doivent être gérées |
