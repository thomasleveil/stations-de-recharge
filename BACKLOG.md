...
| U-7b | ✅ | ~~**Disponibilité TomTom dans les cards mode conduite**~~ — 1er tap sur une card → requête TomTom pour toutes les cards affichées (en parallèle) ; 2ème tap → force-refresh (efface `_availCache`) | ✅ commit `13cee19` — `_driveAhead[]` + `_driveFetched` flag ; handler délégué sur `#dm-cards` ; slot `.dm-avail` dans chaque card |
