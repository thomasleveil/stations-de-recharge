...
| U-7 | ✅ | ~~**Mode conduite**~~ | ✅ — panel `#drive-panel` plein-écran bas ; bouton ⬆ affiché après calcul d'itinéraire si GPS actif ; `enterDriveMode()` / `exitDriveMode()` / `refreshDrivePanel()` ; 10 prochaines stations avec distance, réseau, puissance, bordure or si réseau préféré (F-9b) |
| U-7b | ✅ | ~~**Disponibilité TomTom dans les cards mode conduite**~~ — 1er tap sur une card → requête TomTom pour toutes les cards affichées (séquentiellement, 300 ms entre chaque) ; 2ème tap → force-refresh (efface `_availCache`) | ✅ commits `13cee19` `37badff` — `_driveAhead[]` + `_driveFetched` flag ; handler délégué sur `#dm-cards` ; slot `.dm-avail` dans chaque card ; fetch séquentiel pour éviter le rate limiting |
